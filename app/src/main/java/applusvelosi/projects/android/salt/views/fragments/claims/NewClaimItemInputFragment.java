 ...
 */
public class NewClaimItemInputFragment extends ItemInputFragment {
    ...
    protected void saveToServer() {
        System.out.println("SALTX savetoserver");
        try {
...
//                    activity.startLoading();
                    System.out.println("SALTX loading start");
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            String tempResult;
                            try{
//                                tempResult = app.onlineGateway.test(attachedFile);
//                                tempResult = app.onlineGateway.saveClaimLineItem(activity.claimItem.jsonize(app), ClaimItem.getEmptyClaimLineItemJSON(app), attachedFile);
                                tempResult = app.onlineGateway.uploadAttachment(attachedFile, activity.claimItem.getAttachments().get(0));
                            }catch(Exception e){
                                e.printStackTrace();
